...
function q(lum,k){ //фнкция приведенной массовой скорости;
    var rMV = Math.pow(((k+1)/2),(1/(k-1)))*lum*Math.pow((1-(k-1)/(k+1)*lum*lum),(1/(k-1)));
    return rMV;
}
function rP(lum,k){ //фнкция приведенного давления;
    var reducedPressure = Math.pow((1-(k-1)/(k+1)*lum*lum),(k/(k-1)));
    return reducedPressure;
}
function rIV(P,k){ //функция приведеной изоэнтропной скорости;
...
function diam(square){ //функция нахождения диаметра (в мм) по извесной площади поперечного сечения канала (в м.кв);
...
function vkp(kk,p,r){//фукция определения критическокй скорости
   var t = 2*(kk/(kk+1))*(p/r);
   var kp = Math.sqrt(t);
   return kp;
}
...
function coloring(x){ //функция раскраски влияющих элементов на применение методики (она таки не заработала и ею надо пожертовать) 
    if(x==0){                                    
...
function vuvod(res,x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11,x12,x13,x14,x15){// функция определения столбца содержащего umax;
    ...
    var fpKp = x1*x2/(x3*rP(1,x3)*x4);//площадь кретического сечения сопла, м.кв;
    ...
    var f3 = (x3/k3*rP(1,x3)/rP(1,k3)*x4*aскр*(1+u)/(x8*x2*qc3))*fpKp; //площадь поперчного сечения камеры смешения
    ...
    var lc1//длинна свободной сруи, мм;
    ...
    var lk = 8*d3; //длина цилиндрической камеры смешения для струйных аппаратов 
                    //(выбирается в пределах 6-10 диамтров камеры смешения), мм
...
//функция отрисовки таблицы результатов расчета струйного компресора;
function resu(x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11,x12,x13,x14,x15,x16,x17,x18,x19,x20,x21,x22,x23,x24,x25,x26,x27,x28,
    ...
    var parameters = {
        ...
        7: ["Cкорость рабочего потока в подводящем трубопроводе","м/с", "V<sub>1</sub>",x7.toFixed(3)],
        ...
        18: ["Порядковый номер столбца, в котором находится максимальное значение коеффициента эжекции","-", "n",x18],
        ...
        21: ["Макисиальный коэффициент эжекции","-", "U",x21],
...
calc.onclick = function(){
    ...
    // проверка применимости методики рассчета основной расчет и вывод результатов в таблицу
    ...
    if ((kc.toFixed(1)<=2.5)&&(kc.toFixed(1)>=1.2)){
        ...
        var uI=vuvod(uu2,g1,a1kr,k1,p1,ppn,ro1,v1,p3,qPN,k2,a2kr,p2,β,v3,r);//вызов функци нахождения максимального значения
        //console.log(uI);
        ...
        resu(kc,kp,ro1,ro2,q1,g1,v1,a1kr,a2kr,tet,ppn,lumPN,qPN,pNkr,pPs,kp1,kp2,uI.n,uI.qc3,uI.qн2,uI.u,uI.V2,uI.dkr,uI.dp1,uI.dкам,uI.dc,uI.lc,uI.lk,uI.ld,time11,time21,times,uu2);// отрисовываем таблицу результатов рассчета
        ...
        coloring(1); //функция подсветки влияющих ячеек и полей
                                    ...
                                    p11.innerHTML = "Применима методика рачета эжектора."
                                    results.appendChild(p11);
                                    coloring(0); //функция  подсветки влияющих ячеек и полей
                                                }
            else {
                    p11.style.backgroundColor = "red";
                    p11.innerHTML = "Применима методика рачета инжектора."
                    results.appendChild(p11);
                    coloring(0);//функция подсветки влияющих ячеек и полей
                }
    };
